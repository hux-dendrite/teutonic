(function () {
  ...
  function historyShardRow(item, index) {
    var view = TeutonicDashboardV1.shardPresentation(item), decision = TeutonicDashboardV1.decisionPresentation(item), row = document.createElement("tr"), td = cell(row, "", "history-shards-cell"), panel = document.createElement("div"), reason = document.createElement("div"), reasonLabel = document.createElement("strong"), reasonCopy = document.createElement("div"), reasonSummary = document.createElement("p"), reasonDetail = document.createElement("span"), heading = document.createElement("strong");
    row.className = "history-shards-row"; row.id = "history-shards-" + String(item.challenge_id || "evaluation").replace(/[^a-zA-Z0-9_-]/g, "") + "-" + index; row.hidden = true; row.setAttribute("role", "region"); row.setAttribute("aria-label", "Evaluation details for " + (item.challenge_id || index + 1)); td.colSpan = 9; panel.className = "history-shards-panel";
  ...
  function makeHistoryRowExpandable(row, details, item) {
    row.classList.add("history-row"); row.tabIndex = 0; row.setAttribute("aria-expanded", "false"); row.setAttribute("aria-controls", details.id); row.setAttribute("aria-label", "Show details for evaluation " + (item.challenge_id || "")); row.title = "Click to show evaluation details";
    function toggle(event) { if (event.type === "click" && event.target.closest("a,button")) return; if (event.type === "keydown" && event.key !== "Enter" && event.key !== " ") return; if (event.type === "keydown") event.preventDefault(); var expanded = row.getAttribute("aria-expanded") === "true"; row.setAttribute("aria-expanded", expanded ? "false" : "true"); row.setAttribute("aria-label", (expanded ? "Show" : "Hide") + " details for evaluation " + (item.challenge_id || "")); row.title = expanded ? "Click to show evaluation details" : "Click to hide evaluation details"; details.hidden = expanded; }
