(function () {
  ...
  function renderEvaluation(d) {
    ...
    if (!ev) { card.dataset.active = "false"; text("eval-title", d.queue && d.queue.length ? "NEXT CHALLENGE QUEUED" : "NO ACTIVE CHALLENGE"); el("eval-title").title = d.queue && d.queue.length ? d.queue[0].hotkey || "" : ""; text("eval-meta", d.queue && d.queue.length ? "HOTKEY " + short(d.queue[0].hotkey, 16, 8) : "THE VALIDATOR IS READY FOR THE NEXT MODEL"); text("eval-stage", "WAITING"); text("eval-percent", "0%"); el("eval-progress").style.width = "0%"; return; }
