...
class CloudflareR2TokenGateway:
    ...
    def _permission_group_id(self) -> str:
        if self._permission_id is None:
            groups = self._request(
                "GET", f"/accounts/{self.account_id}/tokens/permission_groups"
            )
            matches = [
                group["id"]
                for group in groups
                if group.get("name") == R2_BUCKET_WRITE_PERMISSION
            ]
            if len(matches) != 1:
    ...
    def create_parent_token(self, token_name: str) -> ParentToken:
        ...
        resource = (
            f"com.cloudflare.edge.r2.bucket.{self.account_id}_"
            f"{self.jurisdiction}_{self.bucket}"
        )
    ...
    def parent_token_exists(self, token_id: str) -> bool:
        result = self._request("GET", f"/accounts/{self.account_id}/tokens")
        return any(item.get("id") == token_id for item in result)
