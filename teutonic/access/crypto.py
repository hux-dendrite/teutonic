...
class SecretCipher:
    """Application-level encryption for parent token values stored in PostgreSQL."""

    def __init__(self, key: bytes) -> None:
        if len(key) != SecretBox.KEY_SIZE:
            raise ValueError("controller secret encryption key must contain 32 bytes")
    ...
    def decrypt(self, ciphertext: bytes) -> str:
        return self._box.decrypt(ciphertext).decode()


class MailboxCipher:
    """Sign credential envelopes and sealed-box encrypt them to miner Ed25519 keys."""
    ...
    def encrypt_for_hotkey(self, envelope: Mapping[str, Any], hotkey: str) -> bytes:
        ed25519_public = decode_ss58_public_key(hotkey, expected_network=self.network)
    ...
    def create_ciphertext(self, *, hotkey: str, **envelope_fields: Any) -> tuple[bytes, dict]:
        envelope = self.build_envelope(hotkey=hotkey, **envelope_fields)
